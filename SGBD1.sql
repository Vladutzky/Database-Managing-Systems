...
CREATE OR REPLACE PROCEDURE ex_09(nationalitate_param IN VARCHAR2) IS
  ...
  CURSOR c_jucatori IS      --cursor care selecteaza numele jucatorului
    ...
    WHERE m.id_meci = c.id_meci and c.id_meci = a.id_meci and m.id_oaspete = e.id_echipa
          and j.id_echipa = e.id_echipa and j.nationalitate = a.tara and j.nationalitate = nationalitate_param and c.tip_cartonas = 'Rosu';  --condi?iile aplicate
  jucator_count NUMBER := 0;
  prea_multi EXCEPTION;  --definirea de exceptii
  niciunul EXCEPTION;
BEGIN
  OPEN c_jucatori;
  LOOP
    ...
    jucator_count := jucator_count + 1;    --un contor care sa ajute la lansarea exceptiilor
  END LOOP;
...
BEGIN
  IF INSERTING OR UPDATING THEN
    -- Verificam daca exista înregistrari în tabelul Competitii
    SELECT COUNT(*) INTO nr FROM Competitii;
    IF nr = 7 THEN
      RAISE_APPLICATION_ERROR(-20001,'Prea multe competitii');
    END IF;
  ELSIF DELETING THEN
    -- Verificam daca mai exista înregistrari în tabelul Competitii dupa stergere
    SELECT COUNT(*) INTO nr FROM Competitii;
    IF nr = 7 THEN
